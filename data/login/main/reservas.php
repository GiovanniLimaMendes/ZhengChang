    ...
    if ($result_verificar->num_rows > 0) {
        ...
        if($mysqli->query($sql) === TRUE){
            ...
            try {
                ...
                $mail->Body    = 'Reserva da ' . $sala . ' realizada com sucesso. <br>' . "Dia: " . $data . "<br> Das: " . $inicio . "<br> Ate: " . $final . "<br> Motivo da reuniao: " . $motivo . "<br> Responsavel pela reserva: " . $responsavel . "<br> Sala: " . $sala . "<br> Observacao: " . $observacao;
...
if(isset($_POST['reservarCarro'])){
    ...
    $sql_verificar = "SELECT * FROM reservaCarro WHERE data = '$data' AND carro = '$carro'";
    $result_verificar = $mysqli->query($sql_verificar);

    // Verifica se já existe uma reserva com a mesma data e sala

    if ($result_verificar->num_rows > 0) {
        echo "<script>alert('Carro já reservado nessa data.');</script>";

    } else {

        // Query SQL para inserir os dados na tabela

        $sql = "INSERT INTO reservaCarro (email, responsavel, motivo, data, inicio, final, carro, observacao) VALUES ('$email', '$responsavel', '$motivo', '$data', '$inicio', '$final', '$carro', '$observacao')";

        
        if($mysqli->query($sql) === TRUE){
            echo "<script>alert('Carro reservado com sucesso.');</script>";
            ...
            try {
                ...
                $mail->setFrom('giovanni.mendes@calibras.com.br');
                $mail->addAddress($email);
                $mail->addAddress('joel.santos@calibras.com.br');
                $mail->addReplyTo('giovanni.mendes@calibras.com.br');
    
                $mail->isHTML(true);
                $mail->Subject = 'Confirmacao reserva de carro';
                $mail->Body    = 'Reserva do ' . $carro . ' realizada com sucesso. <br>' . "Dia: " . $data . "<br> Das: " . $inicio . "<br> Ate: " . $final . "<br> Motivo da reserva: " . $motivo . "<br> Responsavel pela reserva: " . $responsavel . "<br> Carro: " . $carro . "<br> Observacao: " . $observacao;
